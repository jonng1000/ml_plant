...
for classes in list_classes:
    ...
    small.to_csv(first + '_GO.txt', sep='\t')
    data['AraCyc annotation'] = np.nan

##############################################################################
# Creates a dataframe showing gene counts for each GO term and its opposite
# and saves it
grp = 1
mega_lst = []
for classes in list_classes:
    first, second = classes.keys()
    mega_lst.append([grp, first, len(classes[first])])
    mega_lst.append([grp, second, len(classes[second])])
    grp += 1
record = pd.DataFrame.from_records(mega_lst,
                                   columns=['grp', 'GO class', 'genes']
                                   )
record.index.name = 'id'
record.to_csv('records.txt', sep='\t')
