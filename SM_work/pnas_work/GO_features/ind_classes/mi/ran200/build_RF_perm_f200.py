...
Edited from build_RF_perm_f1000.py in
/mnt/d/GoogleDrive/machine_learning/GO_features/ind_classes/output/
Only uses 200 features.
Builds features block by block, with a RF model.
Uses permutation importance to determine the importance of features.
...
ml_data = PERM_FILE.split('_perm')[0] + '_GO.txt'
CLASS_LABELS = 'AraCyc annotation'
# Things I need to modify
RUNS = 100 # number of runs for ml model
# Output variables
# File to save scores
scores_file = PERM_FILE.split('_perm')[0] + '_build_s.txt'

##############################################################################
# Working code from here on, try not to modify it
# Get top X features
data = pd.read_csv(PERM_FILE, sep="\t", index_col=0)
only_data = data.drop(columns=['perm', 'itr'])
top = only_data.mean().sort_values(ascending=False)
col_descend = ['perm', 'itr'] + list(top.index)
data_descend = data.loc[:, col_descend]
