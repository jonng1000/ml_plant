...
No random features. Create permutations for permutation test.
Edited from rf_feature_extract_v3_p.py in
D:\GoogleDrive\machine_learning\GO_features\ind_classes\output
Edition the above script, because when the number of features is
small (e.g. selecting from a larger set), some features may not
be continuous, so script needs to correct for that, otherwise
scaling non-existent continous features will produce an error.
"""
import sys
import pandas as pd
...
# Checks to see if df has continuous features
condition = df.select_dtypes(include='float64').empty
if condition:
    print('WARNING: DATAFRAME HAS NO FLOATS')
...
for i in range(runs):
    ...
    a_perm_df = pd.DataFrame(array_data, columns=labels)
    a_perm_df.index.name = 'perm'
    a_perm_df.insert(loc=0, column='itr', value=i)
    perm_results.append(a_perm_df)
...
df_perm = pd.concat(perm_results)
df_perm.reset_index(inplace=True)
df_perm.index.name = 'idx'
df_perm.to_csv(perm_file, sep='\t')
