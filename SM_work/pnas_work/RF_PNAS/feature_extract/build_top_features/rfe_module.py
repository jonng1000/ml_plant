...
Based on rf_feature_extract.py, for importing. Slightly different from
original file
"""
...
# Code to write in another script when importing this script
# file = '../fully_processed.txt'
# runs = 100
# class_labels = 'AraCyc annotation'
# scores_file = 'test_rf_scores.txt'
# feat_impt_file = 'feat_impt_orig.txt'
...
def balancing(train_features, train_labels, labels):
    '''
    ...
    df_0 = df_train_combined[df_train_combined[labels] == 0]
    df_1 = df_train_combined[df_train_combined[labels] == 1]
    # Assumes minority class is the desired class, and is the second item
    # in the array
    sample_0 = df_0.sample(n=df_train_combined[labels].value_counts().loc[1])
...
# df = read_df(file)
# df['random_1'] = np.random.random(size = len(df))
# df['random_2'] = np.random.random(size = len(df))
# df['random_3'] = np.random.random(size = len(df))
# SM_data, GM_data = sep_df(df, class_labels)
