...
Testing version of rf_feature_extract.py
"""
...
scores_file = 'test_rf_scores.txt'
...
'''
# Exploring rfe class and methods from sklearn
##############################################################################
# Gets importance for each feture
rf.feature_importances_
# Out[17]: 
# array([0.00897249, 0.04298505, 0.0113811 , ..., 0.00122766, 0.00258242,
#        0.00128163])

# Running RFE to taking only best scoring subset
# Step size is 100, ran it at the default step size of 1, but doesn't
# finish after a few min, so I think it takes a while, should run overnight
rfe = RFE(rf, 100, step=100)
rfe = rfe.fit(X_train_scaled, y_train)
rfe.support_
# Out[22]: array([ True,  True,  True, ..., False, False, False])
len(rfe.support_)
# Out[23]: 9537
rfe.ranking_
# Out[24]: array([1, 1, 1, ..., 2, 3, 3])
len(rfe.ranking_)
# Out[25]: 9537
rfe.ranking_ == 1
# Out[26]: array([ True,  True,  True, ..., False, False, False])
rfe.ranking_[rfe.ranking_ == 1]
# Out[29]: 
...
len(rfe.ranking_[rfe.ranking_ == 1])
# Out[30]: 100

# Plots graph of the impoortance of all features
plt.plot(np.sort(rf.feature_importances_)[::-1])
#Out[51]: [<matplotlib.lines.Line2D at 0x1e5c55d8390>]

descending_features = np.sort(rf.feature_importances_)[::-1]
top1000 = descending_features[:1000]
plt.plot(top1000)
top200 = descending_features[:200]
plt.plot(top200)
top100 = descending_features[:100]
plt.plot(top100)
##############################################################################

##############################################################################
# Test rfe using step size of 1, build model based on top 100 features only,
# Score seems better than PNAS?

print('Started rfe', get_time())
rfe = RFE(rf, 100)
# Will still give results for all features
rfe = rfe.fit(X_train_scaled, y_train)
rfe_hat = rf.predict(X_test_scaled)
rfe_f1 = f1_score(y_test, rfe_hat)
rfe_pre = precision_score(y_test, rfe_hat)
rfe_re = recall_score(y_test, rfe_hat) 
print('Finished rfe', get_time())

# rfe.ranking_
# Out[87]: array([ 1,  1,  1, ...,  7,  1, 73])

# len(set(rfe.ranking_))
# Out[79]: 9438
...
# rfe_f1
# Out[84]: 0.8421052631578948

# rfe_pre
# Out[85]: 0.8888888888888888
