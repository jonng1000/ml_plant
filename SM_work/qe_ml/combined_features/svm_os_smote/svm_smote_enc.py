...
X_train, X_test, y_train, y_test = train_test_split(df_features, df_targets,
                                                    test_size=.2)
sm = SMOTENC(categorical_features=[7, 8, 9, 10, 11, 12])
X_bal, y_bal = sm.fit_resample(X_train, y_train)
X_bal_df = pd.DataFrame(data=X_bal, columns=X_train.columns)
y_bal_df = pd.DataFrame(data=y_bal, columns=['Category'])
df_train_balanced = pd.concat([X_bal_df, y_bal_df], axis=1)
...
X_tcs_df = pd.DataFrame(data=X_train_cont_scaled, columns=X_train_cont.columns)
