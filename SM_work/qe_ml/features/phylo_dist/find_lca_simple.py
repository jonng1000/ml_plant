...
gene ID, LCA for each gene ID, and category (GM/SM/no_label)
'''
...
# Creates genes_taxon_df, which contains the various taxa from taxon_list above,
# and shows the lca of each gene
genes_taxon_df = pd.DataFrame.from_dict(genes_taxon_dict, orient='index')
genes_taxon_df.rename(columns={0: 'taxon'}, inplace=True)
...
genes_taxon_df['Category'] = np.nan
m1 = genes_taxon_df.index.isin(priGO)
m2 = genes_taxon_df.index.isin(secGO)
genes_taxon_df['Category'] = genes_taxon_df['Category'].mask(m1, 'GM')
genes_taxon_df['Category'] = genes_taxon_df['Category'].mask(m2, 'SM')
genes_taxon_df['Category'].fillna('no_label', inplace = True)
genes_taxon_df.index.name = 'genes'
genes_taxon_df.to_csv('genes_lca.txt', sep='\t')
