...
FILE = '/mnt/d/GoogleDrive/machine_learning/ml_go_workflow/all_data/ath_ppi_clusters.txt'
OUTPUT = 'ath_ppi_clusters_info.txt'
...
temp_lst = ['Network clusters, ppi cluster id (pid)'] * (len(df) - 1)
lst_values = ['Network clusters'] + temp_lst
df.insert(loc=0, column='Type', value=lst_values)

# Temp list for Description column
temp_lst = ['Cluster id, PPI network'] * (len(df) - 1)
lst_values = ['Cluster size, PPI network'] + temp_lst
