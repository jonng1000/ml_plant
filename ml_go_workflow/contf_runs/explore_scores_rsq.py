...
Explore scores from ml workflow, for continuous features
Saves high scoring job ids to a file
"""
...
DATA_FOLDER = './output/'
OUTPUT = 'high_labels.txt'
OUTPUT2 = 'labels_0_4.txt'
...
# 96 high scoring cont features
high = df_orig.loc[df_orig['oob_r_sq'] >= 0.7, :]
# 149 at least avg scoring cont features
at_least_avg = df_orig.loc[df_orig['oob_r_sq'] >= 0.5, :]
# 175 R sq >= 0.4 cont features
labels04 = df_orig.loc[df_orig['oob_r_sq'] >= 0.4, :]

labels = high.reset_index().loc[:, 'class_label']
labels.index.name = 'id'
labels.to_csv(OUTPUT, sep='\t')

labels_selected = labels04.reset_index().loc[:, 'class_label']
labels_selected.index.name = 'id'
labels_selected.to_csv(OUTPUT2, sep='\t')
