...
Customised for go terms, use random search to optimise hps  and a specific
 number of cores
Pickles saved model to a file

Modified from rf_go_v2.py in
~/machine_learning/ml_go_workflow/go_runs/fixed_hps
"""
import sys
import numpy as np
import pandas as pd
...
NUM = 5  # Number of iterations for random search
CORES = 4  # Num of cores for rf
NCORES = 2 # Num of cores for random search
...
hp_file = OUTPUT_FOLDER + temp_class_label + '_hp.txt'
...
# Container to hold important info which will be saved after script is finished
model_scores = []
fi_values = []
hp_results = []
...
hp_sr = RandomizedSearchCV(estimator=clf, param_distributions=model[2],
                           n_iter=NUM, scoring='f1', cv=inner_kf,
                           n_jobs=NCORES)
...
# Appending score
# CHANGE HERE
y_pred = np.argmax(hp_sr.best_estimator_[model[0]].oob_decision_function_,
                   axis=1)
...
run = pd.Series([model[0], hp_sr.best_estimator_[model[0]].oob_score_,
                 f1, pre, re, time_start, time_end],
                index=['model_name', 'oob_accuracy',
                       'oob_f1', 'oob_precision', 'oob_recall',
                       'time_start', 'time_end'])
model_scores.append(run)
# Appending feature importance
fi = hp_sr.best_estimator_.named_steps[model[0]].feature_importances_
...
hp_df.insert(0, column='model_name', value=model[0])
