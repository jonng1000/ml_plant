...
# Better than mediocore scores
# 248 out of 1379
better = combined_df.loc[combined_df['oob_f1'] > 0.5, :]
...
melted = pd.melt(combined_df, id_vars=['class_label', 'model_name'],
...
for each in score_types:
    ...
    sns.lineplot(data=data_used, x='class_label', y='value',
                 ci=None, marker='o', linestyle='', sort=False)
    ax.set_xticklabels([])
