...
Shows oob f1 score
Random data have been shuffled 100 times
"""
...
DATA_FOLDER2 = './output_ran100/'
DATA_FOLDER3 = './output_ran100_add/'
FILE = 'G:/My Drive/machine_learning/GO_labels/sort_GO_gene_counts.txt'
OUTPUT = 'GO_term_desc_shuffled.txt'
FIG = 'line_71_compare_shuffed.pdf'
...
ran_add_c_hp = combine(DATA_FOLDER3)

full_ran = pd.concat([ran_c_hp, ran_add_c_hp], axis=0)
full_ran = full_ran.reset_index(drop=True)

# Insert column showing if hp optimisation, chosen hps or
# default hps were used
c_hp.insert(loc=1, column='randomised', value='no')
full_ran.insert(loc=1, column='randomised', value='yes')

combined_df = pd.concat([c_hp, full_ran], axis=0)
...
ran_data = data_used.loc[data_used['randomised'] == 'yes', :]
real_data = data_used.loc[data_used['randomised'] == 'no', :]

fig, ax = plt.subplots()
fig, ax = plt.subplots(figsize=(15, 7))
sns.lineplot(data=real_data, x='GO_desc', y='value', hue='randomised',
             ci=None, linestyle='', marker='o', 
             palette=sns.color_palette("hls", 1), sort=False)
sns.boxplot(data=ran_data, x='GO_desc', y='value', hue='randomised',
            order=real_data['GO_desc'])
"""
sns.lineplot(data=ran_data, x='class_label', y='value', hue='randomised',
             ci=95, err_style='bars', linestyle='', marker='o', sort=False)
"""
