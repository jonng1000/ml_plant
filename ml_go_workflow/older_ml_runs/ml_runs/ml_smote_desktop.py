...
workflow without data leakage. Only ColumnTransformer and RandomForestClassifier
have the n_jobs parameter. SMOTE used

For use in desktop, does simpler ml runs, random forest uses all cores
Modified from ml_bcw_shp.py

Crashes on desktop, modify and run on workstation
"""
import numpy as np
import pandas as pd
...
# Input variable
data_file = 'ml_data_label.txt'  # ml data file
...
sm = SMOTENC(categorical_features=bool_array, random_state=42, n_jobs=-1)
...
sample_sizes, train_score, valid_score = learning_curve(clf, 
               ...
               verbose=1, cv=10, scoring='f1', n_jobs=-1)
...
# Playing around with different probability thresholds to see how it
# affects the scores, results copied to a text file in the same folder
# as this script
threshold = 0.5
