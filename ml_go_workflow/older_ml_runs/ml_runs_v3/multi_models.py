...
ML_FILE = 'slightly_bigger2_test.txt' #''ml_dataset_dc.txt'
FI_FILE = 'feature_importance.txt'
SCORES_FILE = 'model_scores.txt'
go_term = sys.argv[1]
class_label = 'go_' + sys.argv[1]
...
logr = LogisticRegression(random_state=42, class_weight='balanced')
...
logr_grid = {
    'logr__C': loguniform(1e-4, 1e4),
}

lsv_grid = {
    'lsv__C': loguniform(1e-4, 1e4),
}
...
models = [(ada_grid, ada, ada_output, 'ada'), (brf_grid, brf, brf_output, 'brf'),
          (logr_grid, logr, logr_output, 'logr'),
          (lsv_grid, lsv, lsv_output, 'lsv'),
