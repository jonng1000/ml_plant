...
Run adaboost with pipeline and other objects to perform the entire machine 
...
Improved version of ada_16cl.py
"""
import sys
import os
import pandas as pd
...
for a_file in os.listdir(DATA_FOLDER):
    ...
    data = pd.read_csv(file_path, dtype=dtype_dict, sep='\t', index_col=0, nrows=1000)
    ...
    grid = {
        MODEL_NAME + '__n_estimators': [50],
        MODEL_NAME + '__learning_rate': [1],
    }
    '''
    grid = {
        MODEL_NAME + '__n_estimators': [50, 100, 200],
        MODEL_NAME + '__learning_rate': [0.6, 1, 1.4],
    }
    '''
    ...
    gd_sr = GridSearchCV(estimator=clf, param_grid=grid,
                         verbose=1, scoring='f1', cv=2, n_jobs=120)
