...
df.index.name = 'Gene'
df = df.rename(columns={0: 'agi_cluster_id', 1: 'agn_cluster_size'})
'''
# Original number of features
df.shape
Out[178]: (22894, 2)
'''
filtered_df = df.loc[df['agn_cluster_size'] > 1, :]
filtered_df = pd.get_dummies(filtered_df, columns=['agi_cluster_id'])
...
filtered_df.drop(columns=['agn_cluster_size']).sum(axis=1).unique()
