...
# The script uses SVR model for peptides (RBF, C=1, epsilon=0.0007)
# The model is stored as pickle (../models/IPC2_protein_75_SVR_18.pickle)
#
...
if __name__ == '__main__':
    ...
    try: 
        ...
        print("e.g.\npython3 %s ../models/IPC2_protein_75_SVR_19.pickle ../datasets/IPC2_protein/IPC2_protein_25.csv /tmp/pred.csv"%sys.argv[0])
        print("\npython3 %s ../models/IPC2_protein_75_SVR_19.pickle ./ipc2_lib/examples/GCA_000027325.1_ASM2732v1_protein.faa /tmp/pred.faa"%sys.argv[0]) 
        print("\npython3 %s ../models/IPC2_protein_75_SVR_19.pickle ALALAKTWKWDDDD /tmp/pred.faa\n"%sys.argv[0]) 
    ...
    if csv_file.endswith('.csv'):
        ...
        print("Input file:\t"+csv_file+' (%s proteins)'%str(len(validation_dataset))) 
    ...
    #second possible input (FASTA as in ./ipc2_lib/examples/ directory)
        ...
        fasta_str = ''
        for n in range(len(validation_tab)):
            header = validation_tab[n][1]+'||isoelectric point (IPC2 SVR protein model): '+predictions[n]
    ...
    #last chance: plain AA input
        ...
        if len(seq)!=len(org_seq):
            ...
            header = '>'+header+'||isoelectric point (IPC2 SVR protein model): '+predictions[0]
            fasta_str = '%s\n%s\n'%(header, validation_tab[0][0])
            
            print("Input sequence: "+seq)
            print('Output file:\t'+output_file+os.linesep)    
            print('Isoelectric point (IPC2 SVR protein model): '+predictions[0])
